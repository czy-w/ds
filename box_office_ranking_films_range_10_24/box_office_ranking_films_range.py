...
def read_csv_and_process(file_path):
    ...
    box_office = [float(box_office.replace('\u200c', '')[:-1]) for box_office in df.iloc[:, 2].tolist()]
...
def collect_all_movies_data():
    ...
    """
    all_movies = []
    file_paths = [os.path.join('box_office_ranking_films_range_10_24', f) for f in os.listdir('box_office_ranking_films_range_10_24') if f.endswith('.csv')]
    file_paths.sort()
...
def plot_horizontal_bar(movie_names, box_office, year, fig_size=(20,32)):
    """
    ...
    ax.invert_yaxis()
    ax.set_xlabel('票房', fontsize=30, color='gray')  # 设置标签字体大小
    ax.set_ylabel('电影名', fontsize=30, color='gray')
    ax.set_title(f'{year}年票房前20电影', fontsize=60, color='white', fontweight='bold',bbox=dict(facecolor='red', edgecolor='red', boxstyle='round,pad=0.5'),x=0.4)
    ...
    for i, v in enumerate(box_office_sorted):
        ax.text(v + 0.05, i, f"{v}亿", color='green', fontweight='bold', va='center')
        rank = i + 1  # 计算排名，从1开始
...
def plot_top_50_overall(all_movies, fig_size=(20,32)):
    """
    ...
    ax.set_xlabel('票房', fontsize=30, color='gray') #,position=(0, 0.5))
    ax.set_ylabel('电影名', fontsize=30, color='gray') #,position=(0, 0.5))
    ax.set_title('2011-2024年票房前50电影汇总', fontsize=60, color='white', fontweight='bold',bbox=dict(facecolor='red', edgecolor='red', boxstyle='round,pad=0.5'),x=0.4)
    ...
    for i, v in enumerate(box_office):
        ...
        rank = rank_dict[i]
        ax.text(v + 0.05, i, f"{v}亿 ({years[i]})",color='green', fontsize=30, fontweight='bold', va='center')
...
def generate_video_with_top_50_overall():
    ...
    # 增加每一帧的停留时间
    for file_path in file_paths:
        movies_data = read_csv_and_process(file_path)[:20]
    ...
    clip.write_videofile('box_office_video_summary.mp4', codec='libx264')
