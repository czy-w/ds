...
def plot_horizontal_bar(movie_names, box_office, year, fig_size=(40,40)):
    ...
    ax.invert_yaxis()
    ax.set_xlabel('数量', fontsize=50, color='gray')
    ax.set_ylabel('球员', fontsize=50, color='gray')
    ax.set_title(f'截至2024年12月24日NBA历史{year}排名', fontsize=60, color='white', fontweight='bold',bbox=dict(facecolor='red', edgecolor='red', boxstyle='round,pad=0.5'),x=0.4)
...
def generate_video_with_top_50_overall():
    ...
    # all_frames.extend([summary_frame]*10)  # 汇总帧重复5次以延长显示时间
