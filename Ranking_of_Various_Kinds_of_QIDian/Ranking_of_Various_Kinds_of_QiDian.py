...
def read_csv_and_process(file_path):
    ...
    df = df.head(50)
    movie_names = [name.replace('\u200c', '') for name in df.iloc[:, 2].tolist()]
...
def plot_horizontal_bar(movie_names, box_office, year, fig_size=(38,38)):
    ...
    ax.invert_yaxis()
    ax.set_xlabel('月票数', fontsize=30, color='gray')
    ax.set_ylabel('书名', fontsize=30, color='gray')
    ax.set_title(f'{year}年起点月票数前50书籍', fontsize=60, color='white', fontweight='bold',bbox=dict(facecolor='red', edgecolor='red', boxstyle='round,pad=0.5'),x=0.4)
...
def plot_top_50_overall(all_movies, fig_size=(38,38)):
    ...
    """
    logging.debug(f"movies overall: {all_movies}")
    plt.rcParams.update({'font.size': 30})  # 调整此值以适应您的需要
    # 获取并根据票房降序排序前50的电影
    max_values_dict = defaultdict(lambda: (0, ''))

    # 遍历列表并填充/更新字典
    for name, value, year in all_movies:
        if value > max_values_dict[name][0]:
            max_values_dict[name] = (value, year)

    # 将字典转换为列表，并按照数值从大到小排序
    unique_sorted_data = sorted([(name, details[0], details[1]) for name, details in max_values_dict.items()], key=lambda x: x[1], reverse=True)

    # 获取前50个非重复的作品
    top_50_movies_sorted = unique_sorted_data[:50]    
    ...
    ax.set_xlabel('月票数', fontsize=30, color='gray') #,position=(0, 0.5))
    ax.set_ylabel('书名', fontsize=30, color='gray') #,position=(0, 0.5))
    ax.set_title('2005-2024年起点月票年度总数前50书籍汇总', fontsize=60, color='white', fontweight='bold',bbox=dict(facecolor='red', edgecolor='red', boxstyle='round,pad=0.5'),x=0.4)
...
def generate_video_with_top_50_overall():
    ...
    # 增加每一帧的停留时间
    for file_path in file_paths:
        movies_data = read_csv_and_process(file_path)[:50]
    ...
    clip.write_videofile('Ranking_of_Various_Kinds_of_QiDian.mp4', codec='libx264')
